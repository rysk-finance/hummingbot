...
def public_rest_url(path_url: str, domain: str = "100x_perpetual"):
    base_url = CONSTANTS.PERPETUAL_BASE_URL if domain == "100x_perpetual" else CONSTANTS.TESTNET_BASE_URL
    return base_url + path_url


def private_rest_url(path_url: str, domain: str = "100x_perpetual"):
    base_url = CONSTANTS.PERPETUAL_BASE_URL if domain == "100x_perpetual" else CONSTANTS.TESTNET_BASE_URL
    return base_url + path_url


def wss_url(endpoint: str, domain: str = "100x_perpetual"):
    base_ws_url = CONSTANTS.PERPETUAL_WS_URL if domain == "100x_perpetual" else CONSTANTS.TESTNET_WS_URL
...
def build_api_factory(
    ...
    api_factory = WebAssistantsFactory(
        ...
        rest_pre_processors=[
            ...
            Class100xPerpetualRESTPreProcessor(),
        ])
    return api_factory
...
def build_api_factory_without_time_synchronizer_pre_processor(throttler: AsyncThrottler) -> WebAssistantsFactory:
    api_factory = WebAssistantsFactory(
        throttler=throttler,
        rest_pre_processors=[Class100xPerpetualRESTPreProcessor()])
    return api_factory
