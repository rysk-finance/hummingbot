...
class Class100xPerpetualWebUtilsUnitTests(unittest.TestCase):
    ...
    def test_rest_url_main_domain(self):
        ...
        generated_url = web_utils.public_rest_url(path_url)
        self.assertEqual(expected_url, generated_url)
    ...
    def test_rest_url_testnet_domain(self):
        ...
        self.assertEqual(
            expected_url,
            web_utils.public_rest_url(path_url, domain=CONSTANTS.TESTNET_DOMAIN),
        )

    def test_wss_url_main_domain(self):
        path_url = "/TEST_PATH_URL"

        expected_url = f"{CONSTANTS.PERPETUAL_WS_URL}{path_url}"
        self.assertEqual(expected_url, web_utils.wss_url(path_url))

    def test_wss_url_testnet_domain(self):
        path_url = "/TEST_PATH_URL"

        expected_url = f"{CONSTANTS.TESTNET_WS_URL}{path_url}"
        self.assertEqual(
            expected_url,
            web_utils.wss_url(path_url, domain=CONSTANTS.TESTNET_DOMAIN),
        )
